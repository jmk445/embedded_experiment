...
void turn_on_led(int led1, int led2, int led3, int led4) {
  ...
  if (led3 == 1) val |= 0x100000;
  if (led4 == 1) val |= 0x800000;
...
int is_button_pushed(int btn) {
  ...
  if (btn == 4) {
    volatile unsigned int input = *PORT_A_INPUT;
...
int main(void)
{
  ...
  while(1) {
    ...
    if (is_button_pushed(2)) {
      turn_off_led(1, 0, 0, 1);
    }
    if (is_button_pushed(3)) {
      turn_on_led(0, 1, 1, 0);
    }
    if (is_button_pushed(4)) {
      turn_off_led(0, 1, 1, 0);
    }
  }
